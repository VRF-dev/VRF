 ...
 * @dev     constuctor(<other arguments>, address _vrfCoordinator, address _tokenAddress)
 * @dev       VRFConsumerBase(_vrfCoordinator, _tokenAddress) public {
 * @dev         <initialization with other arguments goes here>
 * @dev       }
 * @dev   }
 *
 * @dev The oracle will have given you an ID for the VRF keypair they have
 * @dev committed to (let's call it keyHash), and have told you the minimum VOR
 * @dev price for VRF service. Make sure your contract has sufficient VOR, and
...
abstract contract VRFConsumerBase is VRFRequestIDBase {
   ...
   * @param _fee The amount of VOR to send with the request
  ...
  {
    Token.transferAndCall(vrfCoordinator, _fee, abi.encode(_keyHash, _seed));
    ...
    // successful VOR.transferAndCall (in VRFCoordinator.randomnessRequest).
  ...
  VORTokenInterface immutable internal Token;
  ...
  constructor(address _vrfCoordinator, address _tokenAddress) public {
    vrfCoordinator = _vrfCoordinator;
    Token = VORTokenInterface(_tokenAddress);
  }
...
contract VRFCoordinator is VRF, VRFRequestIDBase {
  ...
  VORTokenInterface internal Token;
  BlockHashStoreInterface internal blockHashStore;

  constructor(address _token, address _blockHashStore) public {
    Token = VORTokenInterface(_token);
  ...
  struct Callback { // Tracks an ongoing request
    ...
    // Amount of VRF paid at request time. Total VRF = 1e9 * 1e18 < 2^96, so
  ...
  struct ServiceAgreement { // Tracks oracle commitments to VRF service
    ...
    uint96 fee; // Minimum payment for oracle response. Total VRF=1e9*1e18<2^96
    bytes32 jobID; // ID of corresponding VOR job in oracle's DB
  }
  ...
  mapping(address /* oracle */ => uint256 /* VRF balance */)
   ...
   * @param _fee minimum VRF payment required to serve randomness
   * @param _oracle the address of the VOR node with the proving key and job
   * @param _publicProvingKey public key used to prove randomness
   * @param _jobID ID of the corresponding VOR job in the oracle's db
   */
    ...
    external
  {
    ...
    require(_fee <= 1e9 ether,
      "you can't charge more than all the VRF in the world, greedy");
    serviceAgreements[keyHash].fee = uint96(_fee);
    emit NewServiceAgreement(keyHash, _fee);
  }

  /**
   * @notice Called by VRF Token.transferAndCall, on successful VRF transfer
   *
   ...
   *
   * @param _sender address: who sent the VRF (must be a contract)
   * @param _fee amount of VRF sent
  ...
  /**
   * @notice creates the VOR request for randomness
   *
   ...
   * @param _feePaid Amount of VRF sent with request. Must exceed fee for key
  ...
  function randomnessRequest(
    ...
    internal
    sufficientToken(_feePaid, _keyHash)
  {
    ...
    assert(_feePaid < 1e27); // Total VRF fits in uint96
  ...
  /**
   * @notice Called by the VOR node to fulfill requests
   *
  ...
  /**
   * @dev Allows the oracle operator to withdraw their VRF Token
   * @param _recipient is the address the funds will be sent to
   * @param _amount is the amount of VRF Token transferred from the Coordinator contract
   */
   ...
   */
  modifier sufficientToken(uint256 _feePaid, bytes32 _keyHash) {
    require(_feePaid >= serviceAgreements[_keyHash].fee, "Below agreed payment");
    _;
  }

/**
   * @dev Reverts if not sent from the VRF token
   */
  modifier onlyVRFToken() {
    require(msg.sender == address(Token), "Must use VRF token");
    _;
  }
